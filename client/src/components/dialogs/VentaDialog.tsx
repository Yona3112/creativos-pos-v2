...
import { imprimirTicketTermico } from "@/utils/ticketTermico";
...
export default function VentaDialog({ open, onClose, cart, subtotal, impuesto, total, onSuccess }: VentaDialogProps) {
  ...
  const crearVenta = trpc.ventas.create.useMutation({
    onSuccess: (data) => {
      ...
      if (data && generarNumero.data && configuracion) {
        const datosVenta = {
          ...
          total,
          metodoPago,
          efectivo: metodoPago === "Efectivo" || metodoPago === "Mixto" ? parseFloat(efectivo) || 0 : undefined,
          ...
          nombreTienda: configuracion.nombreTienda || "Creativos Gift Shop",
          rtnTienda: configuracion.rtn || undefined,
          direccionTienda: configuracion.direccion || undefined,
          telefonoTienda: configuracion.telefono || undefined,
        };
        
        // Descargar PDF de la factura
        descargarFacturaPDF(datosVenta);
        
        // Imprimir ticket térmico (88mm)
        setTimeout(() => {
